...
| Keycloak | `keycloak` | `staging-keycloak` | `28080 -> 8080`, `28443 -> 8443` | Runs in `start-dev` mode against the `postgres` service over `host.docker.internal` (requires `KC_DB_*`/`KEYCLOAK_DB_*` vars to match `postgres/.env.staging` and the target database to exist — see `make db.create` below). Bootstrap admin credentials are `KEYCLOAK_ADMIN` and `KEYCLOAK_ADMIN_PASSWORD`. |
